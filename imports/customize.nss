...
// PeaZip is a primary folder tool and follows Code when installed.
modify(mode="multiple" type="dir"
	where=path.exists(peazip_exe) && path.exists(code_exe)
...
modify(mode="multiple" type="dir" where=this.id==id.cut
		&& !path.exists(peazip_exe) && path.exists(code_exe)
